...
$Query = $DB->query("select *  from  `{$INFO[DBPrefix]}discountsubject`  where dsid='".$Subject_id."' and subject_open=1 and start_date<='" . date("Y-m-d",time()) . "' and end_date>='" . date("Y-m-d",time()) . "' limit 0,1");
$Num      = $DB->num_rows($Query);
if ($Num<=0)
	$FUNCTIONS->sorry_back('../../index.php',"活動主題關閉");

while ($Rs    = $DB->fetch_array($Query) ){
	$template = $Rs['template'];
	$tpl->assign("Subject_name",          $Rs['subject_name']);              //主题名字
	$tpl->assign("Subject_content",       $Rs['subject_content']);           //主题内容
	$tpl->assign("remark",       $Rs['remark']);
...
if ($Num>0){
	...
	while ( $ProNav = $DB->fetch_array($arrRecords)){
		...
		$sale_p_array[$i][sale_name] = $ProNav['salename_color']!="" ? "<font color=".$ProNav['salename_color'].">".$sale_p_array[$i][sale_name]."</font>" : $sale_p_array[$i][sale_name] ;
		...
		$Sql_s      = "select *  from `{$INFO[DBPrefix]}storage` where goods_id=" . intval($sale_p_array[$i][gid]);
