...
//---------------<!-- Banner Start -->-----------------------------------------------
$Sql = " select adv_id,adv_title,adv_left_url,title_color from `{$INFO[DBPrefix]}advertising` where adv_type=5 and adv_display = 1 and (start_time='' or start_time<='" . time() . "') and (end_time='' or end_time>='" . time() . "') order by point_num asc  ";
$Query = $DB->query($Sql);
