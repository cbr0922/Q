...
//if ( intval($INFO['float_adv_id'])!=0 && intval($INFO['float_radio'])==1 ) {
	...
	$Sql = " select adv_id,adv_left_url,adv_right_url,adv_width,adv_height,adv_content,point_num,adv_left_img,adv_right_img from `{$INFO[DBPrefix]}advertising` where adv_display = 1 and adv_type=11  and (start_time='' or start_time<='" . time() . "') and (end_time='' or end_time>='" . time() . "') order by adv_idate desc limit 0,1 ";
	$Query = $DB->query($Sql);
