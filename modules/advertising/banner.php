...
//---------------<!-- Banner Start -->-----------------------------------------------
$Sql = " select * from `{$INFO[DBPrefix]}advertising` where adv_type=3 and adv_display = 1 and (start_time='' or start_time<='" . time() . "') and (end_time='' or end_time>='" . time() . "') order by orderby,point_num asc limit 0,1 ";
$Query = $DB->query($Sql);
...
$tpl->assign("adv_url", urlencode($adv_url)); //adv_url
