...
$Sql = " select * from `{$INFO[DBPrefix]}advertising` where adv_display = 1 and adv_type=10 and  adv_tag='Banner' and (start_time='' or start_time<='" . time() . "') and (end_time='' or end_time>='" . time() . "') order by orderby";
$Query = $DB->query($Sql);
...
while($Result = $DB->fetch_array($Query)){
	...
	$adv_array[$i]['img_m'] = $Result['adv_banner'];
	$adv_array[$i]['adv_left_img'] = $Result['adv_left_img'];
	$adv_array[$i]['adv_right_img'] = $Result['adv_right_img'];
...
$tpl->display("adv_banner_full.html");
?>
