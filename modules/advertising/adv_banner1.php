...
$Sql = " select * from `{$INFO[DBPrefix]}advertising` where adv_display = 1 and adv_type=6 and  adv_tag='Banner' and (start_time='' or start_time<='" . time() . "') and (end_time='' or end_time>='" . time() . "') order by orderby";
$Query = $DB->query($Sql);
