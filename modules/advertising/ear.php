...
if (intval($INFO['ear_radio'])==1 && intval($INFO['ear_adv_id'])>0){
	$Sql = "select adv_id,adv_left_url,adv_right_url,adv_width,adv_height,adv_content,point_num,adv_left_img,adv_right_img from `{$INFO[DBPrefix]}advertising` where adv_display=1 AND adv_type=2 and (adv_left_img!='' or adv_right_img!='') and adv_id='".intval($INFO['ear_adv_id'])."' limit 0,1";
	$Query = $DB->query($Sql);
	$Num = $DB->num_rows($Query);
	if ($Num>0){
		$DB->query("update `{$INFO[DBPrefix]}advertising` set point_num=point_num+1 where adv_id=".intval($INFO['ear_adv_id']));  
		...
		*/
		//$Ear_object_left  = "<a href='".$adv_url_left."' target='_blank'>".$Ear_object_left."</a>";
		//$Ear_object_right = "<a href='".$adv_url_right."' target='_blank'>".$Ear_object_right."</a>";
