...
class Cart{
	...
	**/
	...
	將商品放入購物車
	**/
	function getShoppingGoods($gid,$info,$goods_type = "",$key=""){
		global $DB;
		global $INFO,$FUNCTIONS;
		$goods_array = array();
		$Query = $DB->query("select g.gid,g.ifxygoods,g.ifpack from `{$INFO[DBPrefix]}goods` g where gid=".intval($gid)." and g.ifchange!=1 and g.ifpub=1 limit 0,1 ");
		$Num   = $DB->num_rows($Query);
		if ($Num>0) {
			$goods_array = array();
			$Rs = $DB->fetch_array($Query);
			$goods_array['ifinstall'] = intval($info['ifinstall']);
			$goods_array['gid'] = intval($gid);
			...
			$goods_array['count'] = intval($info['count']);
			...
			if(is_array($_GET['changegid']) && $goods_type==""){
				...
				foreach($_GET['changegid'] as $k=>$v){
					...
					$this->setCart($v,$changegoods,"changegoods",$key);
					$z++;
				}
			}
			//組合商品
			...
			while ($Rs=$DB->fetch_array($Query)){
				...
				$this->setCart($Rs['gid'],$pack_array,"packgoods",$key);
			}
	...
	**/
	function getCart($key=""){
		...
		$PRODUCT = new PRODUCT;
		if($key!="")
			$subSql = " where key='" . $key . "'";
		 $Sql = "select s.*,g.* from `{$INFO[DBPrefix]}shopping` as s inner join `{$INFO[DBPrefix]}goods` as g on s.gid=g.gid " . $subSql . "";
		$Query = $DB->query($Sql);
		$Num   = $DB->num_rows($Query);
		$cart_array = array();
		if ($Num>0) {
			while($Rs = $DB->fetch_array($Query)){
				$cart_array[$Rs['pkey']][$Rs['gkey']]['gkey'] = $Rs['gkey'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['gid'] = $Rs['gid'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['bn'] = $Rs['bn'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['provider_id'] = $Rs['provider_id'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['goodsname'] = $Rs['goodsname'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['Js_price'] = $Rs['Js_price'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['detail_id'] = $Rs['detail_id'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['detail_bn'] = $Rs['detail_bn'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['detail_name'] = $Rs['detail_name'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['detail_des'] = $Rs['detail_des'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['storage'] = $PRODUCT->checkStorage($Rs['gid'],$Rs['detail_id'],$Rs['good_color'],$Rs['good_size']);
				$cart_array[$Rs['pkey']][$Rs['gkey']]['unit'] = $Rs['unit'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['good_color'] = $Rs['good_color'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['good_size'] = $Rs['good_size'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['smallimg'] = $Rs['smallimg'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['temp_price'] = $Rs['temp_price'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['price'] = $Rs['price'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['point'] = $Rs['point'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['count'] = $Rs['count'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['trans_type'] = $Rs['trans_type'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['special_trans_money'] = $Rs['special_trans_money'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['trans_special'] = $Rs['trans_special'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['iftransabroad'] = $Rs['iftransabroad'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['ifpresent'] = $Rs['ifpresent'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['ifxygoods'] = $Rs['ifxygoods'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['xygoods'] = $Rs['xygoods'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['xygoods_color'] = $Rs['xygoods_color'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['xygoods_size'] = $Rs['xygoods_size'];
				$cart_array[$Rs['pkey']][$Rs['gkey']]['xygoods_des'] = $Rs['xygoods_des'];
			}
			//print_r($cart_array);
			return $cart_array;
		}
	...
	**/
	function setCart($goodsitems,$pkey = ""){
		...
		if ($pkey != ""){
			...
			}else if($goodsitems['ttype']==1 || $goodsitems['ttype']==2){
				$key = "Y" . $goodsitems['ttype'];
			}else{
	...
	**/
	function addItems($goodsitems = array(),$pkey = ""){
		...
		if($gkey = $this->ifexistItems($goodsitems)){
			...
			$this->goodsGroup[$key][$gkey]['ttype'] = $goodsitems['ttype'];
	...
	**/
	function changeItemsCount($key,$gkey,$count){
		...
		if($isok == 1){
			if($gcount>intval($INFO['buy_product_max_num']))
	...
	**/
	function setMaxStorage($key,$gkey){
		...
		if ($max == -1 || (intval($INFO['buy_product_max_num']) < $max)){
			$max = intval($INFO['buy_product_max_num']);	
		}
		if ($max == -1){
			$max = 0;
		...
		if (is_array($this->goodsGroup[$key])){
				...
				if (intval($v['packgid'])==0){
					...
					*/
					if($v['dsid']==0 || $v['ifds']==0){
						...
						$linshitotal += $total;
						if(intval($v['ifmore'])==0 && $v['ifsale']==0 && trim($v['rebateinfo'])=="" && $v['ifneworold']==0){
							$saletotal += $this->totalPrices + $total;
						}else{
							$nosaletotal += $this->totalPrices + $total;
						}
					}
					/*
					if ($v['ifbonus'] == 1){
			...
			if(is_array($count_array)){
				foreach($count_array as $k=>$v){
					$price_1 = $price_array[$k];
					rsort($price_1);
					//print_r($price_1);
					if($v%$buycount_array[$k]==0){
						$count_1 = 	$v/$buycount_array[$k];
						if($buytype_array[$k]==1){
							
							$total_ds[$k] = $count_1*$buyprice_array[$k];
							$total_price += $total_ds[$k];
						}else{
							
							$total_ds[$k] = round($total_array[$k]*($buyprice_array[$k]/100),0);
							$total_price += $total_ds[$k];
						}
					}else{
						$count_2 = $v%$buycount_array[$k];
						$count_1 = 	($v-$count_2 )/$buycount_array[$k];
						$total_price_2 = 0;
						$total_price_1 = 0;
						for($i=0;$i<$count_2;$i++){
							$total_price_2 += $price_1[$i]	;
						}
						for($i=$count_2;$i<=$v+1;$i++){
							$total_price_1 +=$price_1[$i];
						}
						//echo $total_price_1;
						if($buytype_array[$k]==1){
							$total_ds[$k] = $count_1*$buyprice_array[$k]+$total_price_2;
							$total_price += $total_ds[$k];
						}else{
							$total_ds[$k] = round($total_price_1*($buyprice_array[$k]/100),0)+$total_price_2;
							$total_price +=$total_ds[$k];
						}
						
					}
						if($total_ds[$k]>=$mianyunfei_array[$k] && $mianyunfei_array[$k]>0){
							 $this->manyunfei[$k] = 1;
							 $this->manyunfei[$key] = 1;
						}else{
							 $this->manyunfei[$k] = 0;
							  $this->manyunfei[$key] = 0;	
						}
				}
	...
	**/
	function setTransMoney($man_trans_type,$key,$man_nomal_type = 0,$trans_money = 0,$trans_permoney=0,$mianyunfei=0,$ifabroad=0){
		...
		if($this->sys_trans_type == 1){
			...
			//echo $iffree;exit;
			if ($this->sys_trans[0]['FreeTransMoney'] > 0 && $this->discount_totalPrices >= $this->sys_trans[0]['FreeTransMoney'] && $iffree==1 && $mianyunfei ==0){
				$this->transmoney = 0;
				$this->ifnotrans = 1;
				//$this->transname = "當購買滿" . $this->sys_trans[0]['FreeTransMoney'] . "免運費。";
				$this->ifnotrans = 1;
			}elseif ($mianyunfei > 0 && $this->discount_totalPrices >= $mianyunfei && $iffree==1){
				$this->transmoney = 0;
				$this->ifnotrans = 1;
	...
	*/	
	...
	function setSaleoff($key,$gkey){
		...
		if (intval($this->goodsGroup[$key][$gkey]['detail_id']) == 0 && $this->goodsGroup[$key][$gkey]['nosaleoff']==0){
			...
			$allcount = 0;
			foreach($this->goodsGroup[$key] as $keys=>$values){
				if($values['gid']==$this->goodsGroup[$key][$gkey]['gid']){
					//echo $values['gid'];
					$allcount+= $values['count'];	
				}
			}
			// $Sql      = "select * from `{$INFO[DBPrefix]}goods_saleoffe` where gid='" . intval($this->goodsGroup[$key][$gkey]['gid']) . "' and mincount<=" . $allcount . " and (maxcount>=" . $allcount . " or maxcount=0) order by soid desc limit 0,1";
			if(intval($_SESSION['user_id'])>0)
				 $Sql      = "select m.m_price as price from `{$INFO[DBPrefix]}goods_saleoffe` as s left join `{$INFO[DBPrefix]}member_price` as m on s.soid=m.m_saleoffid where s.gid='" . intval($this->goodsGroup[$key][$gkey]['gid']) . "' and m.m_detail_id='" . intval($this->goodsGroup[$key][$gkey]['detail_id']) . "' and s.mincount<=" . $allcount . " and (s.maxcount>=" . $allcount . " or s.maxcount=0) and m.m_level_id='" . intval($_SESSION['user_level']) . "' order by s.soid desc limit 0,1";
			else
				$Sql      = "select * from `{$INFO[DBPrefix]}goods_saleoffe` where gid='" . intval($this->goodsGroup[$key][$gkey]['gid']) . "'  and mincount<=" . $allcount . " and (maxcount>=" . $allcount . " or maxcount=0) order by soid desc limit 0,1";
