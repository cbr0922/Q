...
class PageItem {
	...
	function PageButton()
	{
		...
		$Pagebutton="<div class='col-xs-12 col-sm-12 col-md-12 padding-0 text-center'><ul class='pagination pagination-simple'>";
		if ($this->CPages > 1) {
			$Pagebutton .= $this->LinktoPage(1, "" , " class='pagination pagination-simple'");/*最前一頁*/
			$Pagebutton .= "   ";			$Pagebutton .= $this->LinktoPage($this->CPages-1, "prev");/*往前一頁*/
		} else {
		...
		if ($this->CPages > 5) {
			if($this->CPages % 5 == 0){
				...
				$fgp = $this->CPages - ($this->CPages % 5) + 1;
			 ...
			 $fgp = 1;
			 $egp = $this->iPages > 5 ? 5 : $this->iPages ;
		 }
