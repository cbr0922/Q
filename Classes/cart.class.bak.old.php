...
class Cart{
	...
	**/
	function addItems($goodsitems = array(),$pkey = ""){
		...
		if($gkey = $this->ifexistItems($goodsitems)){
			...
			$this->goodsGroup[$key][$gkey]['saleoffprice'] = $goodsitems['saleoffprice'];
			$this->goodsGroup[$key][$gkey]['timesale_starttime'] = $goodsitems['timesale_starttime'];
			$this->goodsGroup[$key][$gkey]['timesale_endtime'] = $goodsitems['timesale_endtime'];
	...
	function changeSalePrice($key){
		...
		if (is_array($this->goodsGroup[$key])){
			foreach($this->goodsGroup[$key] as $keys=>$values){
				...
				if ($values['sale_subject'] > 0 && $values['ifsale'] == 1 && $values['ifds'] == 0 && $values['ifchange'] == 0 && $values['ifadd'] == 0){
		...
		if (is_array($this->goodsGroup[$key])){
			foreach($this->goodsGroup[$key] as $keys=>$values){
				if ($values['sale_subject'] > 0 && $values['ifsale'] == 1 && $get_goods_array[$values['sale_subject']] == 1 && $values['ifds'] == 0 && $values['ifchange'] == 0 && $values['ifadd'] == 0){
	...
	**/
	function changeDiscountPrice($key){
		...
		if (is_array($this->goodsGroup[$key])){
			foreach($this->goodsGroup[$key] as $keys=>$values){
				//echo $values['sale_subject'];
				if ($values['dsid'] > 0  && $values['ifchange'] == 0 && $values['ifadd'] == 0){
		...
		foreach($sale_goods_array as $k=>$v){
			if (is_array($this->discountsubject)){
				foreach($this->discountsubject as $kk=>$vv){
					...
					if ($vv['id'] == $k && $vv['mianyunfei']<=$total_goods_array[$k] && $values['ifchange'] == 0){
		...
		if (is_array($this->goodsGroup[$key])){
			foreach($this->goodsGroup[$key] as $keys=>$values){
				if ($values['dsid'] > 0 && $get_goods_array[$values['dsid']] == 1 && $values['ifchange'] == 0){
	...
	**/
	...
	促銷價格優先級活動>多件>單一
	**/
	function setPrice($key,$gkey){
		global $FUNCTIONS;
		
		if($this->goodsGroup[$key][$gkey]['ifds'] == 1 || $this->goodsGroup[$key][$gkey]['nosaleoff']==1 || $this->goodsGroup[$key][$gkey]['ifadd']==1 || $this->goodsGroup[$key][$gkey]['ifchange']==1 || $this->goodsGroup[$key][$gkey]['ifpresent']==1 || $this->goodsGroup[$key][$gkey]['ifbonus']==1)
			return $this->goodsGroup[$key][$gkey]['price'];
		//限時
		if($this->goodsGroup[$key][$gkey]['iftimesale']==1 && $this->goodsGroup[$key][$gkey]['timesale_starttime']<=time() && $this->goodsGroup[$key][$gkey]['timesale_endtime']>=time())
			return $this->goodsGroup[$key][$gkey]['saleoffprice'];
		//買越多
		$price = $this->setSaleoff($key,$gkey);
		if($this->goodsGroup[$key][$gkey]['ifmore']==1)
			return $price;
		$MemberPiceReturn = $FUNCTIONS->MemberLevelPrice($_SESSION['user_level'],$this->goodsGroup[$key][$gkey]['gid'],$this->goodsGroup[$key][$gkey]['detail_id']);
		return $MemberPiceReturn>0 ? $MemberPiceReturn : $this->goodsGroup[$key][$gkey]['price'] ;
