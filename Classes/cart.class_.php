		...
		if ($Num>0) {
			...
			*/
			...
			/*
				...
				if(is_array($_GET['presentgoods']) && $goods_type==""){
				//$Sqld = "delete from `{$INFO[DBPrefix]}shopping` where pregid='" . $gid . "'" . $subSql;
	   			//$Result_Update = $DB->query($Sqld);
					$z = 0;
					foreach($_GET['presentgoods'] as $k=>$v){
						//$Query = $DB->query("select * from `{$INFO[DBPrefix]}goods_present` as gc where gc.gid='" . $gid . "' ");
						//$Rs=$DB->fetch_array($Query);
						$presentgoods['count'] = intval($info['count']);
						$presentgoods['ifgoodspresent'] = 1;
						$presentgoods['pregid'] = $gid;
						$presentgoods['gid'] = intval($v);
						//$presentgoods['mgoodsname'] = "[".$Rs['goodsname']."]";
						$this->getShoppingGoods($v,$presentgoods,"goodspresent",$key);
						$z++;
					}
				}
			}
