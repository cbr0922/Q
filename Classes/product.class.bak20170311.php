...
class PRODUCT{
	...
	**/
	function getProductList($bid=0,$type="",$search_array=array(),$orderby = array(),$showcount=0,$ifpage=0,$ifshowcolor=0,$ifshowmemberprice=0,$ifshowsale=0,$pagetype="1"){
		...
		*/
		...
		if($orderby[0]=="")
			$orderSql = " order by g.goodorder asc,g.storage desc,g.ifalarm asc,g.idate desc ";
		else{
			if ($orderby[0]=="price")
	...
	**/
	function getProductPic($gid,$loop=1,$type=0){
		global $DB,$INFO;
		$Sql_pic    = "select middleimg,bigpic,goodpic_title from `{$INFO[DBPrefix]}good_pic` where goodpic_name<>'' and type='" . $type  ."' and good_id='".intval($gid)."'";
		$Query_pic  = $DB->query($Sql_pic);
		$Num_pic    = $DB->num_rows($Query_pic);
		$i = $loop;
		if ($Num_pic>0){
			while ($Result_pic = $DB->fetch_array($Query_pic))  {
				$Goodpic[$i]['pic'] =   $Result_pic['middleimg'];
	...
	**/
	function getRecProduct(){
		global $DB,$INFO,$FUNCTIONS;
		$Sql = "select g.gid,g.goodsname,g.price,g.bn,g.smallimg,g.pricedesc,g.salename_color,g.intro ,g.alarmnum,g.storage,g.ifalarm,g.middleimg,g.bigimg,g.gimg,g.js_begtime,g.js_endtime,g.ifjs,b.bid,g.sale_name,g.ifxygoods,g.iftimesale,g.timesale_starttime,g.timesale_endtime,g.saleoffprice,g.ifalarm,g.storage,g.ifsaleoff,g.saleoff_starttime,g.saleoff_endtime,g.ifappoint,g.appoint_starttime,g.appoint_endtime  from `{$INFO[DBPrefix]}bclass` b left join `{$INFO[DBPrefix]}goods` g on ( g.bid=b.bid) where g.ifpub=1 and (g.pubstarttime='' or g.pubstarttime<='" . time() . "') and (g.pubendtime='' or g.pubendtime>='" . time() . "') and g.ifrecommend=1 and g.ifbonus!=1 and g.ifpresent!=1 and g.ifxy!=1 and ifchange!=1 and g.ifxy!=1 and b.catiffb=1 and g.ifgoodspresent!=1 order by g.goodorder asc,g.idate desc limit 0,12";
	...
	**/
	function getProductLink($gid,$bid,$ifgl=1){
		...
		if (intval($ifgl)==1){ //判断是否是指定了产品内容，如果没有就把本类产品资料都调出来
			$Sql   = "select g.gid,g.goodsname,g.price,g.smallimg,g.middleimg,g.intro,g.pricedesc,gl.s_gid,g.sale_name,g.storage from `{$INFO[DBPrefix]}goods` g left join `{$INFO[DBPrefix]}good_link` gl  on (g.gid=gl.s_gid) where g.ifpub=1 and (g.pubstarttime='' or g.pubstarttime<='" . time() . "') and (g.pubendtime='' or g.pubendtime>='" . time() . "') and g.ifpresent!=1 and g.ifbonus!=1 and gl.p_gid=".$gid;
		}else{
			$Sql   = "select g.gid,g.goodsname,g.price,g.smallimg,g.middleimg,g.pricedesc,g.intro,g.sale_name,g.storage from `{$INFO[DBPrefix]}goods` g where g.bid=".$bid." and g.gid!=".$gid." and g.ifpub=1 and (g.pubstarttime='' or g.pubstarttime<='" . time() . "') and (g.pubendtime='' or g.pubendtime>='" . time() . "') and g.ifpresent!=1 and g.ifchange!=1 and g.ifxy!=1 and g.ifgoodspresent!=1 and g.ifbonus!=1 order by g.idate desc limit 0,5 ";
		}
		$Query = $DB->query($Sql);
		$i=1;
		$j=0;
	...
	**/
	function getChangeProduct($gid){
		...
		$change_array = array();
		$i = 0;
		$Sql         = "select gl.* ,g.goodsname,g.bn,g.smallimg,g.good_color,g.good_size,g.ifchange,g.ifalarm from `{$INFO[DBPrefix]}goods_change` gl  inner join `{$INFO[DBPrefix]}goods`  g on (gl.changegid=g.gid) where gl.gid=".intval($gid)." and g.ifchange=1 and g.ifpub=1 and (g.pubstarttime='' or g.pubstarttime<='" . time() . "') and (g.pubendtime='' or g.pubendtime>='" . time() . "') order by gl.idate desc ";
		$Query       = $DB->query($Sql);
		$Num         = $DB->num_rows($Query);
			...
			if($Result['gc_string']!=""){
				...
				foreach($gc_arrays as $k=>$v){
					...
					if($goods_Num>0){
						...
						$return_array['goods'][$j]['price'] = $goods_Rs['price'];
						$return_array['goods'][$j]['pricedesc'] = $goods_Rs['pricedesc'];
	...
	**/
	function ViewGidBelate($gid,$type=0){
		...
		$age = $age != '' ? round((time()-strtotime($age))/(24*60*60)/365.25,0) : 0;

		if ($type==1 && $Ifbelate == 1 && (intval($_SESSION['user_id']) == 0 || (intval($_SESSION['user_id']) != 0 && intval($age)<18))){
			return 1;
		}

		if (intval($_SESSION['user_id']) == 0 && $Ifbelate == 1){
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'><script language='javascript'>alert('請先註冊或登入才能觀看。');location.href='../member/login_windows.php?&Url=" . $_SERVER['HTTP_REFERER']. "';</script>";exit;
		}

		if (intval($_SESSION['user_id']) != 0 && intval($age)<18 && $Ifbelate == 1){
			echo "<meta http-equiv='Content-Type' content='text/html; charset=utf-8'><script language='javascript'>alert('未滿18歲不得觀看商品資料。');location.href='" . $_SERVER['HTTP_REFERER'] . "';</script>";exit;
		}
	}
}
?>
