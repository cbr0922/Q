			...
			*/
			...
			/*
				...
				foreach($present_array as $pk=>$pv){
						...
						//設置數量
						$Sql_p         = "select gl.* from `{$INFO[DBPrefix]}goods_present` gl where gl.gid=".$gid." and gl.pregid='".$pv['gid']."' order by gl.idate desc ";
						$Query_p       = $DB->query($Sql_p);
						$Result_p    = $DB->fetch_array($Query_p);
						if($Result_p['count']>0){
							$pack_array['count'] = $Result_p['count']*intval($info['count']);
						}else{
							$pack_array['count'] = intval($info['count']);
						}
	...
	function getCart($key=""){
		...
		if ($Num>0) {
			while($Rs = $DB->fetch_array($Query)){
				...
				//echo "|";
				if($storage<=0 && $Rs['packgid']>0 && ($cur_page=="shopping.php" || $cur_page=="shopping2.php"||$cur_page=="shopping3.php"||$cur_page=="shopping4.php")){
					...
					if ($Rs['ifbonus']==1){
						...
						if($Rs['ifadd']==1){
							...
							$cart_array[$Rs['pkey']][$Rs['gkey']]['promotion_name'] = "滿額加購";
		...
		foreach($this->cart_array[$key] as $k=>$v){
			...
			if($v['goodspresentgid']!=""){
				foreach($thisgoodspresentgid_array as $pk=>$pv){
					//設置數量
					if($pv>0){
					$Sql_p         = "select gl.* from `{$INFO[DBPrefix]}goods_present` gl where gl.gid='".$v['gid']."' and gl.pregid='".$pv."' order by gl.idate desc ";
					$Query_p       = $DB->query($Sql_p);
					$Result_p    = $DB->fetch_array($Query_p);
					if($Result_p['count']>0){
						$v['count'] = $Result_p['count']*intval($v['count']);
					}else{
