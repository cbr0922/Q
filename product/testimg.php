...
$phpThumbBase      = '../phpThumb-master/phpThumb.php';
$bid = intval($_GET['bid']);
...
if($bid == 0){
	...
	//商品列表
	$product_array = $PRODUCT->getProductList(0,$type,array('key'=>$_GET['skey']),array($_GET['order'],$_GET['ordertype']),199,0,1,0,1);
	//屬性
	...
	//if($classinfo_array['top_id']>0){
		...
		//熱賣商品3個
		$hot_array = $PRODUCT->getProductList($bid,$type,array(),array(),3,0,0,0,1);
		//商品列表
		$product_array = $PRODUCT->getProductList($bid,"",array(),array($_GET['order'],$_GET['ordertype']),199,0,$ifshowcolor,0,1);
	//}else{
	//	$class_product_array = $PRODUCT->getClassList($bid,1);	
	//}
}
echo "一共" . $product_array['count'] ."商品";
foreach($product_array['info'] as $k=>$v){
	$product_array['info'][$k]['smallimg']	= htmlentities(phpThumbURL("src=../UploadFile/GoodPic/" . $v['gimg'] . "&q=100&w=200", $phpThumbBase), ENT_QUOTES);
}
