...
$currFolder	=	$sPath;
$upAllowed=true;
$ffilter="data_files";	//filter here
$sUploadedFile="";
$MaxFileSize = 500000000;
$AllowedTypes = "|txt|csv|xls|xlsx";
//$AllowedTypes = "*";
  ...
  for ($i=0; $i<count($aItem); $i++)
    ...
    if($sItem=="."||$sItem=="..")
      ...
      if(!$fIsDirectory)
        ...
        $bDisplay=false;
        $sExt=getExt($sItem);
        if($ffilter=="data_files")	//custom
          {
          if ($sExt=="csv" || $sExt=="txt" || $sExt=="xls" || $sExt=="xlsx") $bDisplay=true;
          }
        else if($ffilter=="flash")
          {
        ...
        if($bDisplay)
          ...
          echo "<td><img src='images/".$sIcon."'></td><td><input type=radio id=chkFile".$nIndex." name=chkFile onclick='checkFile()'></td>";
...
<html>
<head>
<title><?php echo FILEMANAGER_1;?></title>
...
function checkFile() {
	...
	parent.document.getElementById("<?php echo $targetField?>").value = (sResult);
	}
...
function getAction()
  ...
  if(sQueryString.indexOf("=")==-1)
    {//no querystring
    sAction="dataFileManager.php?ffilter="+document.getElementById("selFilter").value;
    }
  else
    {
    sAction="dataFileManager.php?"+sQueryString+"&ffilter="+document.getElementById("selFilter").value
    }
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  return sAction;
  }
