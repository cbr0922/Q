...
class orderClass{
	...
	**/
	function setOrderState($state_value,$state_type,$order_id,$remark,$ifadmin=1,$ifdetailstate=0){
		...
		if ($flag==1){
			...
			if ($state_value==4 && $state_type==1){
				$Sql_detail = "select (point*hadsend) as thepoint from `{$INFO[DBPrefix]}order_detail` where order_id='" . $order_id . "' group by order_id";
				$Query_detail  = $DB->query($Sql_detail);
				$Rs_detail=$DB->fetch_array($Query_detail);
				...
				*/
				...
				$FUNCTIONS->AddBonuspoint(intval($User_id),intval($discount_totalPrices),1,"訂單" . $Order_serial,2,$order_id);
				...
				if ($Num_old>0 ){
					...
					$levelSql = "select * from `{$INFO[DBPrefix]}user_level` where vip_money<='" . $discount_totalPrices . "' and level_id<>'" . $user_level . "' and vip_money>0 order by vip_money desc limit 0,1";
					...
					$u_sql = "select sum(discount_totalPrices) as discount_totalPrices from `{$INFO[DBPrefix]}order_table` where user_id='" . trim($User_id) . "' and pay_state=1 and order_state=4 and createtime>='" . (time()-60*60*24*365) . "' and createtime<='" . time() . "'";	
				...
				if ($discount_totalPrices>0 && intval($INFO['ordermoney'])<=$discount_totalPrices && intval($INFO['ordermoney'])>0){
					$orderpoint = intval($discount_totalPrices/intval($INFO['ordermoney']))*intval($INFO['orderpoint']);
			...
			$DB->query($u_Sql);
			$Sql = "select * from `{$INFO[DBPrefix]}order_table` where order_id='" . $order_id . "'";
			$Query = $DB->query($Sql);
			...
			// $result = file_get_contents($fb_url . "?op=updateorderstatus&moid=" . $MallgicOrderId . "&utvorderid=" . $Order_serial . "&orderstatus=" . $State . "&token=" . $token);
	...
	設置詳細訂單狀態
	**/
	function setOrderDetailState($state_value,$state_type,$order_id,$detail_id,$remark,$hadsend=-1,$ifsingle=0,$ifadmin=1,$backPrice=0,$backBouns=0){
		...
		$Sql_had = "";
		$Sql_order = "select od.*,ot.user_id from `{$INFO[DBPrefix]}order_detail` as od inner join `{$INFO[DBPrefix]}order_table` as ot on od.order_id=ot.order_id where od.order_id = '" . intval($order_id) . "' and od.order_detail_id='" . intval($detail_id) . "'";
		$Query_order  = $DB->query($Sql_order);
		$Rs_order=$DB->fetch_array($Query_order);
		$User_id     = $Rs_order['user_id'];
		...
		if ($flag==1){
			if ($state_type==2&&$state_value==4 && ($backPrice!=0 || $backBouns!=0)){
				...
				$iSql = "insert into `{$INFO[DBPrefix]}buypointrefund`(bank,bankcode,account,acountname,u_id,state,refundtime,point,remark,refundtype,order_id)values('','','','','" . $User_id . "',0,'" . time() . "','" . $backPrice . "','" . $remark . "','" . intval($_POST['refundtype']) . "','" . $order_id . "')";	
				$DB->query($iSql);
				...
				$back_Sql = "update `{$INFO[DBPrefix]}order_table` set bonuspoint=bonuspoint-" . intval($backBouns) . ",refundtype='" . intval($_POST['refundtype']) . "' where order_id='" . $order_id . "'";
				$DB->query($back_Sql);
				$FUNCTIONS->AddBonuspoint(intval($User_id),intval($backBouns),2,"訂單" . $Order_serial,1,$order_id);	
				//print_r($_POST);exit;
				/*
			...
			if($state_value==4&&$state_type==1){
				$us_Sql = "update `{$INFO[DBPrefix]}goods` set gid='" . $gid ."' where salenum=salenum+1";
				$DB->query($us_Sql);
			}
			//if($state_value==1 && $state_type==1)
			//		$sub_Sql = ",transport_state=12";
			$Sql_update_o = "update `{$INFO[DBPrefix]}order_detail` set " . $state_name . "='" . $state_value . "'" . $Sql_had . "" . $sub_Sql . " where order_detail_id='" . $detail_id . "'";
			$DB->query($Sql_update_o);
			if($partstate==10000){
	...
	**/
	function getUserOp($order_id){
		...
		$op_array = array(array("申請取消","javascript:changeOrderState(2,1," . $order_id . ")"),array("付款","../shopping/payorder.php?orderno=" . $Rs_order['order_serial']),array("申請退貨","javascript:location.href='kefu_add.php?order_serial=" . $Rs_order['order_serial'] . "&type=back';"));//array("申請退貨","javascript:changeOrderState(5,3," . $order_id . ")"
		...
		//}
		if($Rs_order['order_state']==1 && $Rs_order['pay_state']==1 && $Rs_order['transport_state']==2 ){
			$button_array[count($button_array)] = $op_array[2];
		}
		if($Rs_order['transport_state']==1 && time()<=$order_time){
	...
	**/
	function getUserDetailOp($order_id,$detail_id){
		...
		$order_time = $Rs_order['createtime']+7*24*60*60;
		$op_array = array(array("申請退貨","javascript:changeOrderState(5,3," . $order_id . "," . $detail_id . ")"));
		...
		if($Rs_order['detail_transport_state']==1 && time()<=$order_time && $Rs_order['ifbonus']==0){
			$button_array[count($button_array)] = $op_array[1];
		}
		return $button_array;
	}
