...
if ($Num>0){
	...
	while ($Rs = $DB->fetch_array($Query)) {
		...
		$OrderList[$i]['city']     = $Rs['city'];		$OrderList[$i]['post']   = $Rs['post'];
		$OrderList[$i]['reid']     = $Rs['reid'];
		$OrderList[$i]['receiver_tele']     = MD5Crypt::Decrypt ( $Rs['receiver_tele'], $INFO['tcrypt']);		$OrderList[$i]['receiver_mobile']     = MD5Crypt::Decrypt ( $Rs['receiver_mobile'], $INFO['mcrypt']);
		$i++;
	}
}
...
$tpl->display("shopping_ajax_receiver.html");
?>