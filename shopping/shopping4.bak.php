...
foreach($items_array as $k => $v){
	...
	if ($v['ifbonus']==0 && $v['ifpresent']==0 && $v['Js_price']==0 && $v['iftimesale']==0 && $v['ifchange']==0 && $v['ifsale']==0 && $v['ifadd']==0&& $v['dsid']==0 && $v['rebate']==0){
...
//if ($_POST['Action']=="insert" || $paytype == 0){
	...
	'discount_totalPrices'                     => intval($Cart_discount_totalPrices-$Cart_buypoint),
	...
	*/
	if ($Order_serial){
		...
		foreach($items_array as $item){
			...
			$Sql_Ok = "insert into `{$INFO[DBPrefix]}order_detail` (gid,goodsname,unit,good_color,good_size,goodscount,market_price,price,order_id,point,provider_id,month,detail_id,detail_bn,detail_name,detail_des,xygoods,ifxygoods,ifchange,xygoods_color,xygoods_size,xygoods_des,oeid,memberorprice,memberprice,combipoint,bonuspoint,ifbonus,bn,iftogether,cost,salecontent) values('".$item['gid']."','".$item['goodsname'].$item['rebateinfo']."','".$item['unit']."','".$item['good_color']."','".$item['good_size']."','".$item['count']."','".intval($item['temp_price'])."','".$item['price']."','".$Insert_id."','".$item['point']."','".$item['provider_id']."','".intval($item['month'])."','".intval($item['detail_id'])."','".$item['detail_bn']."','".$item['detail_name']."','".$item['detail_des']."','" . $xygoods . "','" . intval($item['ifxygoods']) . "','" . intval($item['ifchange']) . "','" . $xygoods_color . "','" . $xygoods_size . "','" . $item['xygoods_des'] . "','" . $item['oeid'] . "','" . $item['memberorprice'] . "','" . $item['memberprice'] . "','" . $item['combipoint'] . "','" . $item['bonuspoint'] . "','" . $item['ifbonus'] . "','" . $item['bn'] . "','".$item['iftogether']."','" . $cost . "','" . $salecontent . "')";
		...
		if ($Cart_buypoint > 0){
			$FUNCTIONS->AddBuypoint(intval($_SESSION['user_id']),intval($Cart_buypoint),1,"訂單" . $Order_serial . "使用qMoney",$Insert_id,0,0,1);	
		}
