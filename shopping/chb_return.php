...
$checkno = $_POST['OrderNum'];
if (isset($_POST)){
	if ($_POST['final_result']==1){
		$Sql = "select ot.order_serial,ot.order_id,od.goodsname from  `{$INFO[DBPrefix]}order_table` ot inner join  `{$INFO[DBPrefix]}order_detail` od on (ot.order_id=od.order_id) where ot.order_serial='".trim($_POST[P_OrderNumber])."' ";
		$Query = $DB->query($Sql);
		...
		if ($Num>0){
			...
			$Sql = "UPDATE `{$INFO[DBPrefix]}order_table` SET $db_string WHERE order_serial='".intval($_POST['P_OrderNumber'])."'";
			$Result = $DB->query($Sql);
			$Reprot ="pass";
		}
		else{
...
$Query = $DB->query(" select ot.*,ttime.transtime_id,ttime.transtime_name from `{$INFO[DBPrefix]}order_table` ot left join `{$INFO[DBPrefix]}transtime` ttime on (ot.transtime_id=ttime.transtime_id) where ot.order_id='".trim($_POST[P_OrderNumber])."' ");
$Num       = $DB->num_rows($Query);
...
if ($Reprot == "error"){
	$final_return_PRC = $_POST['final_return_PRC'];
	$final_return_SRC = $_POST['final_return_SRC'];
	$final_return_BankRC = $_POST['final_return_BankRC'];
	if($final_return_PRC=="8" && $final_return_SRC=="204"){
               $error = "交易失敗-->訂單編號重複!";
          }else if($final_return_PRC=="34" && $final_return_SRC=="171"){
               $error =  "交易失敗-->金融上的失敗!";
               $error =  "  銀行回傳碼=[".$final_return_BankRC."]<br>";
          }else{
               $error =  "交易失敗-->請與商家聯絡!";
          }
		  $error = "回覆碼：" . $final_return_PRC . ";" . $final_return_SRC . " 錯誤信息：" .  $error;
?>
