...
///////////// TOOL function ///////////////
  ...
  function s_k_search(act_url){
     ...
     if(act_url){
     	 ss=encodeURI(document.main_store.store_k_word.value)
...
obj_html+="<img src='" + img_url + "' width=0 height=0>";
      }
    }
...
//////
  ...
  function jswinopen(id, action, jwinname, in_x, in_y, in_width, in_height, jswin_arg, ssl_arg) {
    ...
    var img_min_str,img_max_str,img_close_str,dragable_fg,resizeable_fg,reht="";   
    if (ssl_arg && typeof(ssl_arg) != 'undefined') { 
      ssl_arg=ssl_arg.toLowerCase(); 
      if (ssl_arg == "ssl") { reht="https://www.pcstore.com.tw"; }
    }   
    ...
    if (browser.safari) {
      ...
      myscrollLeft = canvas.scrollLeft;
      myscrollTop = canvas.scrollTop;
    }
    ...
    try {
      ...
      var jwin = new JsWindow({id: id, width : jswin_width, Height : jswin_height, title : jwinname, action : winAction, head_imgs: head_imgs});
...
function make_ad_impression_js_code(){
  ...
  function go_sep_print(ecno,subno,odno,odnm,amt,cvslink) {
    ...
    var won_tst=window.open('http://www.pcstore.com.tw/adm/opt/cvs_ebill.htm?ecno='+ecno+'&subno='+subno+'&odno='+odno+'&odnm='+odnm+'&amt='+amt+'&cvslink='+cvslink,'sep_print_winopen_tst','left=100,top=50,width=760,height=1050,resizable=1,scrollbars=1');
    won_tst.focus();
/*
...
//=== member change password--> END
  ...
  function do_order(exopt){
  	var my_ev = "jswinopen('order_detail','/order_manage/order_detail_view.htm?order_id=no&rnd=jrnd','訂單明細','','','','','','ssl');";
  	var nev = my_ev.replace("=no", "=" + event.srcElement.innerHTML.trim());
