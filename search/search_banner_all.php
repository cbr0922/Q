...
$Sql_bclass    = "select bid,catname,pic1,pic2 from `{$INFO[DBPrefix]}bclass` where catiffb=1 and top_id='0' order by catord,bid  asc    ";
$query_bclass  = $DB->query($Sql_bclass);
$num_bclass    = $DB->num_rows($query_bclass);
$i=0;
...
while ($Rs_bclass =  $DB->fetch_array($query_bclass)){
	...
	$ProductListAll[$i]['link'] = $link;
		$j=0;
		$Sql_bclass_2    = "select bid,catname,pic1,pic2 from `{$INFO[DBPrefix]}bclass` where catiffb=1 and top_id='" . $Rs_bclass['bid'] . "' order by catord,bid  asc  ";
		...
		while($Rs_bclass_2 =  $DB->fetch_array($query_bclass_2)){
			...
			$ProductListAll[$i]['sub'][$j]['pic2'] = $Rs_bclass_2['pic2'];
				$z=0;
				$Sql_bclass_3    = "select bid,catname,pic1,pic2 from `{$INFO[DBPrefix]}bclass` where catiffb=1 and top_id='" . $Rs_bclass_2['bid'] . "' order by catord,bid  asc  ";
...
$tpl->display("include_product_class_all.html");
?>
