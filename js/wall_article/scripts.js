...
jQuery(document).ready(function($){
	...
	if ($('#posts-1').exists()) {
		setTimeout(function() {
			$('#posts-1').masonry({
				columnWidth: 250,//文章一塊的寬度加上２０px（注意：如果有設框線要記得連同框線的寬度一起加上，例如一塊是200px，那加上左右兩邊的框線1px就是202px）
				isResizable: true
			});
		}, 800);
	}
	...
	$('.item.type-love.not-loved .img.love-it:before').click(function(){
			loveIt($(this).parent().find('.fy-loves'));	
			return false;
	});
