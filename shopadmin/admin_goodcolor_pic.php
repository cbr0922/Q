...
$Query = $DB->query("select g.gid,g.goodsname,g.good_color from `{$INFO[DBPrefix]}goods` g where g.gid=".intval($Good_id)." ".$Provider_string." limit 0,1");
...
$Query = $DB->query("select p.* from `{$INFO[DBPrefix]}goodcolor` p  where p.good_id=".intval($Good_id)." limit 0,20 ");
$Num   = $DB->num_rows($Query);
...
if ($Num>0){
	...
	while ( $Result   =  $DB->fetch_array($Query)){
		...
		$Goodpic_id_array[$i]       =  $Result['id'];
		$Goodpic_color_array[$i]     =  $Result['color'];
		$Goodpic_pic_array[$i]  =  $Result['pic1'];
		$Goodpic_pic2_array[$i]  =  $Result['pic2'];
		$i++;
	}
}
...
function  checkimage2(){
	//setTimeout('function wait(){}',5000);
	document.upload.photo2.value = trim(document.upload.photo2.value);
	var name = document.upload.photo2.value;
	if(/^.+\.(JPG|GIF|gif|jpg|png)$/i.test(name)){
	if(img)img.removeNode(true);
	img=document.createElement("img");
	img.style.position="absolute";
	...
	*/
	document.getElementById("view2").src          = name;
	document.getElementById("view2").style.width  = showimg_width;
	document.getElementById("view2").style.height = showimg_height;
}
else{ErrImgType()}
}
...
function picsubmit() {
	...
	*/
	...
	filename = document.upload.photo2.value.toLowerCase();
	filext = filename.substring(filename.length-3,filename.length);
	if(filext!="jpg" &&  filext!="gif" && filext!="swf" && filext!="png" ) {
		alert('<?php echo $FUNCTIONS_Pack['Upload_File_FileFormatError_Say'];?>'); //"上傳的圖片格式錯誤,只能上傳gif,jpg,png或swf格式
		return;
	...
	*/
	...
	document.upload.action = "admin_goodcolor_pic_save.php";
	document.upload.Action.value="Addpic";
	document.upload.submit();
...
function delpic(goodpic_id,filename,filename2){
	var pic   = filename ;
	var gpid  = goodpic_id
	if (confirm('<?php echo $Admin_Product[Del_Pic];?>')){  //您是否确认删除该图片
		...
		document.upload.GoodpicName2.value=filename2;
...
<TABLE cellSpacing=0 cellPadding=0 width="97%" align=center border=0>
  <FORM  action='admin_goodcolor_pic_save.php' method=post enctype='multipart/form-data' 	name='upload' onSubmit="return picsubmit()" >
  <input type="hidden" name="Action" >
  <input type="hidden" name="GoodpicName" >
  <input type="hidden" name="GoodpicName2" >
  ...
  <TR>    <TD width="1%" background=images/<?php echo $INFO[IS]?>/left.gif style="background-repeat: repeat-y;" height=319></TD>
					  ...
					  <!--BUTTON_BEGIN-->
                        <TABLE>
                          ...
                          <TR>
							...
							<a href="admin_goods.php?Action=Modi&gid=<?php echo $Good_id?>"><IMG  src="images/<?php echo $INFO[IS]?>/fb-cancel.gif" border=0>&nbsp;<?php echo $Basic_Command['Cancel'];//取消?></a>&nbsp; </TD>
							</TR>
						  </TBODY>
						 </TABLE>
					 <!--BUTTON_END-->
                ...
                <TD vAlign=top bgColor=#ffffff height=300>
                  ...
                  width="100%" align=center bgColor=#f7f7f7 border=0>
                    ...
                    <TR>
                      ...
                      顔色：</TD>
                      <TD noWrap align=left><select name="color"><?php if (trim($good_color)!=""){
	...
	?></select>                      
                    ...
                    <TR>
                      <TD noWrap align=right width="18%"> 上傳顔色圖片：</TD>
					  ...
					  <TR>
                      <TD noWrap align=right width="18%"> 上傳商品圖片：</TD>
                      <TD noWrap align=left><input name="photo2" type="file" id="photo2" onChange="setTimeout('checkimage2()',500)">
					  <div id="phototips" class="tips"><?php echo $Admin_Product[UploadIntro] ?></div>	
					  </TD></TR>
                    <TR>
					...
					<TR>
                      <TD noWrap align=right>&nbsp;</TD>
                      <TD noWrap align=left><IMG id=view2  src="../include/blank.gif" border=0></TD>
                    </TR>
                    <TR>
...
?>
<td width=90 height=75 align="center"><?php echo $Goodpic_color_array[$i]?><img src="<?php echo $INFO['site_url']."/".$INFO['good_pic_path']?>/<?php echo $Goodpic_pic_array[$i]?>"><br>
<img src="<?php echo $INFO['site_url']."/".$INFO['good_pic_path']?>/<?php echo $Goodpic_pic2_array[$i]?>"><br>
  <br><input type="button"  class="inputstyle" name="botton" value="<?php echo $Basic_Command["Del"];//删除?>" onClick='javascript:delpic("<?php echo $Goodpic_id_array[$i]?>","<?php echo $Goodpic_pic2_array[$i]?>","<?php echo $Goodpic_pic_array[$i]?>");'></td>
<?php 
  }
}else{
?>
