...
$Sql      = "select  g.*,bc.catname from `{$INFO[DBPrefix]}goods` g  left join `{$INFO[DBPrefix]}bclass`  bc  on (g.bid=bc.bid)    where g.subject_id!=0  ".$Where." ";
$Add      = !empty($_GET['subject_id']) && intval($_GET['subject_id'])!=0 ? " and g.subject_id=".intval($_GET['subject_id']) : "" ;
//$Sql      = $_GET['alarm_recsts']=='DO' ? " select * from goods where ifalarm=1 and alarmnum>=storage  and subject_id!=0 " : $Sql ;
$Sql      = $Sql.$Add."  order by g.idate desc , g.goodorder desc ";
...
$Sql_sub   = " select subject_name,subject_id,subject_open from `{$INFO[DBPrefix]}subject` order by subject_num desc ";
$Query_sub = $DB->query($Sql_sub);
$Table_sub = "
<select name='subject_id'  onchange=\"window.location=('admin_SubjectProject_list.php?subject_id='+this.value);\"   class=\"trans-input\" >
<option value='' selected>--$Admin_Product[AllSubjectProduct]--</option>
";
while ($Rs_sub = $DB->fetch_array($Query_sub)){
	$Table_sub .=  "
     <option value='$Rs_sub[subject_id]' 
   ";
	if ($Rs_sub[subject_id]==$_GET[subject_id])	{
		$Table_sub  .=  "  selected ";
	}
	$Table_sub  .= " > ".$Rs_sub[subject_name]."";
	if (intval($Rs_sub[subject_open])==0){
		$Table_sub  .= "&nbsp;[$Basic_Command[Close]]";
	}
	$Table_sub  .= "</option>";
}
$Table_sub .="</select>";
...
<title><?php echo $JsMenu[Product_Man];//商品管理?>--&gt;<?php echo $JsMenu[Subject];?></title>
...
function toEdit(id,catid){
	...
	if (checkvalue!=false){		
		...
		document.adminForm.act.value="";
   	    document.adminForm.Where.value="Subject";		
		document.adminForm.submit();
	}
}
          ...
          <TR>
            <TD width="50%">
              <TABLE width="90%" border=0 cellPadding=0 cellSpacing=0>
                ...
                <TD class=p12black noWrap><SPAN class=p9orange><?php echo $JsMenu[Product_Man];//商品管理?>--&gt;<?php echo $JsMenu[Subject];?>--&gt;<?php echo $Table_sub?></SPAN></TD></TR></TBODY></TABLE></TD>
        ...
        <FORM name=optForm method=get action="">        
          ...
          <TR>
            <TD align=left colSpan=2 height=31>
              <TABLE class=p12black cellSpacing=0 cellPadding=0 width=255 border=0>
                <TBODY>
                  <TR>
                    <TD align=right width=210 height=31>
                      <INPUT  name='skey'   onfocus=this.select()  onclick="if(this.value=='<?php echo $Admin_Product[PleaseInputPrductName]?>')this.value=''"  onmouseover=this.focus() value='<?php echo $Admin_Product[PleaseInputPrductName]?>' size="30"> </TD>
                    <TD align=middle width=9 height=31>
                      
                      </TD>
                    <TD width=36 height=31 align="left" vAlign=center class=p9black> <INPUT type=image src="images/<?php echo $INFO[IS]?>/t_go.gif" border=0 name=imageField align="absmiddle"> 
                      </TD>
                    </TR>
                  </TBODY>
                </TABLE>
