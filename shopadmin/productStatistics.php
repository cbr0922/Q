...
case "sale":
	...
	$Sql     = " SELECT sum( s.salemoney ) AS smtotal, sum( s.salenum ) AS sntotal , s.gid,g.goodsname FROM `{$INFO[DBPrefix]}saletotal` s  inner join   `{$INFO[DBPrefix]}goods`  g  ON ( g.gid = s.gid )  WHERE s.saledate > '".$Begtime."' AND  s.saledate <= '".$Endtime."'  GROUP BY s.gid order by smtotal desc  LIMIT 0 , 100 ";
break;

case "viewpoint":
    $title   =$ProductVisit_Packs[PointSort];//點閱數排行
	$Sql     = " SELECT sum( point ) AS point_total, gid, goodsname FROM `{$INFO[DBPrefix]}goods`  where  GROUP BY gid order by point_total desc  LIMIT 0 , 100 ";
break;
					   ...
					   <TABLE class=listtable cellSpacing=0 cellPadding=0   width="100%" border=0>
                           <TBODY>
                             <TR align=middle class=row1>
                               ...
                               <TD align="left" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1><?php echo $ProductVisit_Packs[ProductSaleMoney] ?></TD>
                               <TD width="80"  colspan="2" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1> <?php echo $ProductVisit_Packs[ProductSaleNum] ?></TD>
                              </TR>
	             			<?php
	             			if ($Num>0){
	             				$i=1;
						    ...
						    ?>   
                             <TR class=row0>
                               ...
                               <TD height=20 align="left" noWrap><?php echo $Result['smtotal'];?></TD>
                               <TD height=20 colspan="2" align="center" noWrap><?php echo $Result['sntotal'];?></TD>
                               </TR>
   						    <?php
						    ...
						    ?>
							  <TR align="right">
							   <TD height=20 colspan="5" class="p9orange"><br>
							     <br></TD>
							   </TR>
   						    <?php
	             			}else{
						    ?>
							 <TR class=row1>
                               <TD height=20 colspan="5" align=center class="p9orange"><?php echo $Visit_Packs[NoVisit_Say];//没有参与统计的资料?></TD>
                               </TR>
   						    <?php
	             			}
						    ?>
