...
if ($_POST['Action']=='Update' ) {
	...
	if ($Result_Insert)
	{
		...
		*/
		if (intval($_POST['state'])==1 && $oldstate !=1){
			//echo "aaaaaaaaaaa";exit;
			$checkno = randstr(25);
			$Sql = "UPDATE `{$INFO[DBPrefix]}provider` SET checkno='" . $checkno . "' WHERE provider_id=".intval($_POST['Provider_id']);
			$Result_Insert = $DB->query($Sql);
			$Array =  array("provider_name"=>trim($_POST['provider_name']),"provider_thenum"=>trim($_POST['Provider_id']),"provider_loginpassword"=>trim($_POST['provider_loginpassword']),"checkno"=>trim($checkno));
			$SMTP->MailForsmartshop(trim($_POST['provider_email']),"pigangel_yang@aliyun.com",35,$Array);
			$Sql      = "select * from `{$INFO[DBPrefix]}operater` where email<>'' and opid='" . $_POST['PM'] . "' order by lastlogin desc ";
			$Query    = $DB->query($Sql);
			while ($Rs=$DB->fetch_array($Query)) {
				//echo trim($Rs['email']);
				$SMTP->MailForsmartshop(trim($Rs['email']).",".$_POST['provider_email'],"",35,$Array);
			}//exit;
			
		}
		if (intval($_POST['state'])==2 && $oldstate !=2){
			$Array =  array("provider_name"=>trim($_POST['provider_name']),"provider_thenum"=>trim($_POST['provider_thenum']),"provider_loginpassword"=>trim($_POST['provider_loginpassword']));
			$SMTP->MailForsmartshop(trim($_POST['provider_email']),"pigangel_yang@aliyun.com",18,$Array);
			$Sql      = "select * from `{$INFO[DBPrefix]}operater` where email<>'' and opid='" . $PM . "' order by lastlogin desc ";
			$Query    = $DB->query($Sql);
			while ($Rs=$DB->fetch_array($Query)) {
				$SMTP->MailForsmartshop(trim($Rs['email']),"",18,$Array);
