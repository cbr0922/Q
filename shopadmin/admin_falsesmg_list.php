...
if($_POST['act']=="Del"){
	...
	for ($i=0;$i<$Num_bid;$i++){
		$Result =  $DB->query("delete from `{$INFO[DBPrefix]}falsesmg` where id=".intval($Array_bid[$i]));
	}

	$FUNCTIONS->header_location('admin_falsesmg_list.php?pid=' . $_POST['pid']);	
}

$Sql      = "select * from `{$INFO[DBPrefix]}falsesmg` where pid='" . intval($_GET['pid']) . "' order by sendtime desc ";
...
function toDel(){
	...
	if (checkvalue!=false){
		if (confirm('<?php echo $Basic_Command['Del_Select']?>')){
			document.adminForm.action = "admin_falsesmg_list.php";
              ...
              <TABLE width="90%" border=0 cellPadding=0 cellSpacing=0>
                <TBODY>
                  <TR>
                    ...
                    <TD class=p12black noWrap><SPAN class=p9orange><?php echo $JsMenu[Functions];//功能?>--&gt;<?php echo $JsMenu[Shop_Pager];//网店会刊?>--&gt;簡訊列表--&gt;發送失敗</SPAN>
                    </TD>
                </TR></TBODY></TABLE></TD>
                        ...
                        <FORM name=adminForm action="" method=post>
                          ...
                          <TBODY>
                            <TR align=middle>
                              ...
                              <TD width="212"  height=26 align="left" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1>手機號</TD>
                              <TD width="265" align="left" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1> 發送時間</TD>
                              <TD height="26" colspan="5" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1> 發送編號 </TD>
