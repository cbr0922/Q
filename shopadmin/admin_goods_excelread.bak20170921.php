...
/**从第A列开始输出*/ 
	...
	if ($Num_trans > 0){
		...
		//匯入詳細規格
			...
			if($goods_Num>0){
				...
				$update_Sql = "update `{$INFO[DBPrefix]}goods_detail` set detail_name='" . trim($data[getindex("af")]) . "',guojima='" . trim($data[getindex("j")]) . "',orgno='" . trim($data[getindex("k")]) . "',storage='" . trim($data[getindex("ab")]) . "' where goodsno='" . trim($data[18]) . "'";
			  	$DB->query($update_Sql);
				
			}else{
				$update_Sql = "insert into `{$INFO[DBPrefix]}goods_detail`(gid,detail_bn,detail_name,storage,guojima,orgno)values('" . $gid . "','" . trim($data[getindex("ae")]) . "','" . trim($data[getindex("af")]) . "','" . trim($data[getindex("ab")]) . "','" . trim($data[getindex("j")]) . "','" . trim($data[getindex("k")]) . "')";	
				$DB->query($update_Sql);
	...
	//echo $extendbid;exit;
	    ...
	    if(trim($data[getindex("ah")])!=""){
			$ah_array = explode("/",$data[getindex("ah")]);
			if($ah_array[1]<10)
				$ah_array[1] = "0" . $ah_array[1];
			if($ah_array[2]<10)
				$ah_array[2] = "0" . $ah_array[2];
			$salestartdate = $ah_array[0]."-".$ah_array[1]."-".$ah_array[2];
		}else{
			$salestartdate = "";
		}
		if(trim($data[getindex("ai")])!=""){
			$ah_array = explode("/",$data[getindex("ai")]);
			if($ah_array[1]<10)
				$ah_array[1] = "0" . $ah_array[1];
			if($ah_array[2]<10)
				$ah_array[2] = "0" . $ah_array[2];
			$saleenddate = $ah_array[0]."-".$ah_array[1]."-".$ah_array[2];
		}else{
			$saleenddate = "";
		}
		$sql_data = array (
	...
	if(trim($data[19])=="" && trim($data[getindex("ae")])==""){
		$sql_data['storage'] = intval($data[getindex("ab")]);
	}
	if($gid<=0){
