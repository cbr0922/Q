...
if ($_POST['Action']=="Login" && trim($_POST['username'])!="" && trim($_POST['passwd'])!=""  && trim($_POST['type'])==0){
	...
	$Sql   = "select * from `{$INFO[DBPrefix]}administrator` where sa='".trim($_POST['username'])."' and pw='".md5(trim($_POST['passwd']))."'  limit 0,1";
	$Query = $DB->query($Sql);
	$Num   = $DB->num_rows($Query);
	if ($Num>0){
...
if ($_POST['Action']=="Login" && trim($_POST['username'])!="" && trim($_POST['passwd'])!=""  && trim($_POST['type'])==1){
	...
	$Sql   = "select * from `{$INFO[DBPrefix]}operater` where username='".trim($_POST['username'])."' and userpass='".md5(trim($_POST['passwd']))."' and status=1 limit 0,1";
	$Query = $DB->query($Sql);
	$Num   = $DB->num_rows($Query);
	if ($Num>0){
		$Result = $DB->fetch_array($Query);
...
if (($_SERVER[HTTP_HOST]!="localhost" || $_SERVER[HTTP_HOST]!="127.0.0.1") && VersionArea!='big5'){
	$String_content = @file_get_contents("http://www.zendshop.com/report/login_reprot_".VersionArea.".txt","rb");
	if ($String_content && trim($String_content)!=""){
		$String_content_array = explode("\n",$String_content);
	}
}
...
<div id="floater"></div>
    <div id="middle">
      ...
      <p>&nbsp;</p>
      <TABLE width="383" height=158 border=0 align="center" cellPadding=2 cellSpacing=0 class=p9black style="margin-top:35px" >
		          <TBODY>
		            <TR>
