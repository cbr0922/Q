...
$Sql         = "select gl.* ,g.goodsname,g.bn from `{$INFO[DBPrefix]}goods_change` gl  inner join `{$INFO[DBPrefix]}goods`  g on (gl.changegid=g.gid)  order by gl.idate desc ";
$Query       = $DB->query($Sql);
...
if (is_array($gid_array)){
	$sub = " g.gid in (" . implode(",",$gid_array) . ")";	
}
...
$Sql      = "select  g.*,bc.catname from `{$INFO[DBPrefix]}goods` g  left join `{$INFO[DBPrefix]}bclass`  bc  on (g.bid=bc.bid)  where  ".$sub.$Where." ";
              ...
              <TABLE cellSpacing=0 cellPadding=0 border=0>
                <TBODY>
                  <TR>
                    ...
                    <TD class=p12black noWrap><SPAN class=p9orange><?php echo $JsMenu[Product_Man];//商品管理?>--&gt;加購商品</SPAN></TD>
                </TR></TBODY></TABLE></TD>
        ...
        <TBODY>
          <TR>
            <TD vAlign=top height=210>
              <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0 id="orderedlist">
                <TBODY>
                  <TR>
                    <TD bgColor=#ffffff>
                        ...
                        <FORM name=adminForm action='' method=post>
                          ...
                          <TBODY>
                            <TR align=middle>
                              ...
                              <TD width="17%"  height=26 align="left" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black>
                                <?php echo $Admin_Product[PrductClassName];//商品類別名稱?>					  </TD>
                              <TD width="41%" align="left" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black><?php echo $Admin_Product[ProductName];//商品名稱?>&nbsp;</TD>
