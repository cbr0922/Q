...
<TITLE>商品管理--&gt;商品管理--&gt;商品匯入/匯出欄位設定</TITLE>
</HEAD>
              ...
              <TABLE width="90%" border=0 cellPadding=0 cellSpacing=0>
                <TBODY>
                  <TR>
                    ...
                    <TD class=p12black noWrap><SPAN  class=p9orange>商品管理--&gt;商品管理--&gt;商品匯入/匯出欄位設定</SPAN></TD>
                    </TR>
                  </TBODY>
                </TABLE>
              </TD>
...
選擇全部 </TD></TR>
                          <TR>
                            <TD align=right valign="top" noWrap><i class="icon-download-alt" style="font-size:14px"></i> 商品匯出欄位設定：</TD>
                            <TD colspan="3" align=left valign="top" style="padding-right:90px">
                              <?php
					  $goods_excel_out = explode(",",$INFO['goods_excel_out']);
                       foreach($goods_field as $k=>$v){
						?>
                              <input type="checkbox" name="goods_excel_out[]" <?php if(in_array($k,$goods_excel_out)) echo "checked";?> value="<?php echo $k;?>"><?php echo $v;?>
                              <?php   
					   }
					   ?>
                              </TD>
                          ...
                          <TR>
                            <TD align=right valign="top" noWrap><i class="icon-upload-alt" style="font-size:14px"></i> 商品匯入欄位設定：</TD>
                            <TD colspan="3" align=left valign="top" style="padding-right:90px"><?php
					  $goods_excel_in = explode(",",$INFO['goods_excel_in']);
                       foreach($goods_field as $k=>$v){
						?>
                              <input type="checkbox" name="goods_excel_in[]" <?php if(in_array($k,$goods_excel_in)) echo "checked";?>  value="<?php echo $k;?>"><?php echo $v;?>
                              <?php   
					   }
					   ?></TD>
                            </TR>
...
if ( $_POST['Action']=="Modi" ){
	...
	$Ex_Function->save_config( $new = array("goods_excel_out","goods_excel_in"),"conf.global") ;
	$FUNCTIONS->setLog("商品匯入匯出欄位設定");
	...
	echo " <script language=javascript> alert('".$Basic_Command[Back_System_Sucuess]."'); location.href='admin_product_excel_sys.php'; </script>";
}
?>
