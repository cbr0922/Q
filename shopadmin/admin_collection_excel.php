 ...
 */
if ($_GET[Action]=='Excel' && intval($_GET[gc_id])) {
	...
	if ($Num>0){
        ...
        $XML_String .= "<".date("Ymd",time()).">\n";
		$XML_String .= "<CollectionName>".$GCName."</CollectionName>\n";
        $XML_String .= "<CollectionPic>".Filter($gc_pic)."</CollectionPic>\n";
	...
	echo $XML_String;
	$Creatfile = iconv("UTF-8",$ToEncode,$gc_name)."_".date("Y-m-d");
