...
$Sql      = "select d.goodsname,d.price,d.bn,d.packgid,t.order_serial,t.paymentname from `{$INFO[DBPrefix]}user` u left join `{$INFO[DBPrefix]}order_table` t on u.`memberno`=t.`recommendno` left join `{$INFO[DBPrefix]}order_detail` d on t.`order_id` = d.`order_id` WHERE u.`memberno`='".$_GET['memberno']."' AND `createtime` BETWEEN UNIX_TIMESTAMP('".$goods_starttime."') AND UNIX_TIMESTAMP('".$goods_endtime." 23:59:59') AND t.order_state!=3 AND t.pay_state!=2 GROUP BY t.order_serial,t.paymentid,d.gid ORDER BY t.order_serial desc";
...
<TITLE><?php echo $JsMenu[Functions];//功能?>--&gt;會員統計--&gt;會員推薦消費紀錄</TITLE></HEAD>
              ...
              <TABLE width="90%" border=0 cellPadding=0 cellSpacing=0>
                <TBODY>
                  <TR>
                    ...
                    <TD class=p12black noWrap><SPAN class=p9orange><?php echo $JsMenu[Functions];//功能?>--&gt;會員統計--&gt;會員推薦消費紀錄</SPAN>
                    </TD>
                </TR></TBODY></TABLE></TD>
          </TR>
        </TBODY>
          ...
          <input type="hidden" name="Action" value="Search">
					...
					<TR>
						...
						<TD height="30" colspan="2" align=left class=p9black>
							<input type="button" value="匯出" onclick="javascript:location.href='admin_member_recommend_record_csv.php?<?php echo "memberno=" . $_GET['memberno'] . "&goods_starttime=" . $goods_starttime . "&goods_endtime=" . $goods_endtime; ?>'"/>
						</TD>
