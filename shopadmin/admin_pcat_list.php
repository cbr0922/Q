...
if(intval($_GET['top_id'])>0){
	$Query_goods = $DB->query("select * from `{$INFO[DBPrefix]}bclass` where bid=".intval($_GET['top_id'])." limit 0,1");
...
$Sql      = "select * from `{$INFO[DBPrefix]}bclass` where top_id='" . intval($_GET['top_id']) . "' order by catord asc";
...
function toEdit(id,catid){
	...
	if (checkvalue!=false){
		document.adminForm.action = "admin_pcat.php?Action=Modi&top_id=<?php echo $_GET['top_id'];?>&bid="+checkvalue;
...
function toDel(){
	...
	if (checkvalue!=false){
		if (confirm('<?php echo $Basic_Command['Del_Select']?>')){
			document.adminForm.action = "admin_pcat_save.php?top_id=<?php echo $_GET['top_id'];?>";
              ...
              <TABLE cellSpacing=0 cellPadding=0 border=0>
                <TBODY>
                  <TR>
                    ...
                    <TD class=p9orange>
                      <?php echo $JsMenu[Set]?>--&gt;<?php echo $JsMenu[Sys_Set]?>--&gt;<?php echo $JsMenu[Product_Man];//商品管理?>--&gt;<a href="admin_pcat_list.php"><?php echo $JsMenu[Product_Class_List];//商品类别列表?></a><?php if (intval($top_id) > 0){?>--&gt;<?php echo $top_areaname;}?>
                      </TD>
                    </TR>
                  </TBODY>
                </TABLE>
                        ...
                        <TR>
                          <TD align=middle width=79><!--BUTTON_BEGIN-->
                            <TABLE>
                              <TBODY>
                                <TR>
                                  <TD vAlign=bottom noWrap class="link_buttom"><A href="admin_pcat.php?top_id=<?php echo $_GET['top_id'];?>"><IMG  src="images/<?php echo $INFO[IS]?>/fb-new.gif"  border=0>&nbsp;新增同級分類</A></TD>
			  ...
			  <li class="ui-state-default"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>Item 7</li>
					...
					?>
                            <TR class=row0 id="<?php echo $Rs['bid']?>">
                              ...
                              <TD height=26 align="left" noWrap><A href="admin_goods_list.php?bid=<?php echo $Rs['bid']?>"><?php echo $Rs['catname']?></a>
                              <?php
								 if($Rs['catiffb']==0)
									 echo "[未發佈]";
								  ?>
                              ...
                              <?php
								$Sqlc = "select count(*) as totalcount from `{$INFO[DBPrefix]}goods` where extendbid like '%\"".$value."\"%' or bid='" . $Rs['bid'] . "'";
						        $Queryc  = $DB->query($Sqlc);
						        $Resultc = $DB->fetch_array($Queryc);
						        echo intval($Resultc['totalcount']);
							  ?>
							  </TD>
                              <TD align="left" noWrap><a href="admin_pcat.php?top_id=<?php echo $Rs['bid']?>">新增下級分類</a> <a href="admin_pcat_list.php?top_id=<?php echo $Rs['bid']?>">下級分類列表</a></TD>
                              <TD align="left" noWrap><a href="admin_pcat.php?Action=Modi&bid=<?php echo $Rs['bid']?>">編輯</a></TD>
                              </TR>
                            <?php
					$j++;
					$i++;
