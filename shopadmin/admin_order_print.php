...
$Query = $DB->query(" select ot.*,ttime.transtime_id,ttime.transtime_name from `{$INFO[DBPrefix]}order_table` ot left join `{$INFO[DBPrefix]}transtime` ttime on (ot.transtime_id=ttime.transtime_id) where ot.order_id=".intval($Order_id)." limit 0,1");
...
$Receiver_tele    = MD5Crypt::Decrypt ( trim($Rs['receiver_tele']), $INFO['tcrypt']);//$Rs['receiver_tele'];
...
$objActSheet->setCellValue('H2', $orderClass->getOrderState(intval($Order_state),1) ."|". $orderClass->getOrderState(intval($Pay_state),2)."|". $orderClass->getOrderState(intval($Transport_state),3) );
...
$objActSheet->setCellValue('H8', $Invoiceform);
...
if ($Num_user>0){
	...
	$objActSheet->setCellValueExplicit('B13', MD5Crypt::Decrypt ( trim($Result_user['tel']), $INFO['tcrypt']),PHPExcel_Cell_DataType::TYPE_STRING);
...
$objActSheet->setCellValue('H23', "規格");
...
if ($Num > 0){
	while ($Rs = $DB->fetch_array($Query)){
		...
		if($Rs['packgid']>0){
						...
						$xiaoji = $Rs['price']*$Rs['goodscount'];
						$hongli = $Rs['point']*$Rs['goodscount'];
						$chuhuo = intval($Rs['goodscount'])-intval($Rs['hadsend']);
		...
		if($Rs['detail_name']=="")
			$detail =  $Rs['good_color'] . "/" . $Rs['good_size'];
		else
			$detail =  $Rs['detail_name'];
		...
		$objActSheet->setCellValue('H' . $j,$detail);
		$objActSheet->setCellValueExplicit('I' . $j, $Rs['hadsend'],PHPExcel_Cell_DataType::TYPE_STRING);
		$objActSheet->setCellValueExplicit('J' . $j, $chuhuo,PHPExcel_Cell_DataType::TYPE_STRING);
		if($Rs['packgid']==0){
			$Total_detailprice = $Rs['hadsend']!=0 ? abs($Rs['hadsend']*$Rs['price']) : abs($Rs['goodscount']*$Rs['price']) ;
			//$state_say = $FUNCTIONS->OrderStateName($Rs['detail_order_state']) . "[" . $FUNCTIONS->OrderPayState(intval($Rs['detail_pay_state'])) . "]";
			$state_say = $orderClass->getOrderState(intval($Rs['detail_order_state']),1) ."|". $orderClass->getOrderState(intval($Rs['detail_pay_state']),2)."|". $orderClass->getOrderState(intval($Rs['detail_transport_state']),3);
		}else{
			$Total_detailprice = "";
			$state_say = "";
		}
		$objActSheet->setCellValueExplicit('K' . $j, $Total_detailprice,PHPExcel_Cell_DataType::TYPE_STRING);
		$objActSheet->setCellValue('L' . $j,  $state_say);
		$i++;
		$j++;
	}
