...
$Sql      = "select u.username,d.goodsname,d.price,d.bn,d.packgid,t.order_serial,t.paymentname from `{$INFO[DBPrefix]}user` u left join `{$INFO[DBPrefix]}order_table` t on u.`memberno`=t.`recommendno` left join `{$INFO[DBPrefix]}order_detail` d on t.`order_id` = d.`order_id` WHERE t.`recommendno`='".$_GET['memberno']."' AND `createtime` BETWEEN UNIX_TIMESTAMP('".$goods_starttime."') AND UNIX_TIMESTAMP('".$goods_endtime." 23:59:59') AND t.order_state!=3 AND t.pay_state!=2 GROUP BY t.order_serial,t.paymentid,d.gid ORDER BY t.order_serial desc";

//echo $Sql;

$content = ",".$INFO['company_name']."\n\n";
$content .= "會員".$_GET['memberno']."推薦消費紀錄,".$goods_starttime."~".$goods_endtime."\n\n";
...
header("Content-Disposition: attachment; filename='會員".$_GET['memberno']."推薦消費紀錄".$goods_starttime."_".$goods_endtime.".csv'");
