...
$P_Sql= "select b.*,u.email,u.username,u.other_tel,sum(b.point) as sumpoint from `{$INFO[DBPrefix]}bonuspoint` as b inner join `{$INFO[DBPrefix]}user` as u on b.user_id=u.user_id where b.endtime>='" . $overtime . "' and b.endtime<='" . ($overtime+60*60*24) . "' and b.saleorlevel=1 and b.usestate=0 group by b.user_id";
$P_Query = $DB->query($P_Sql);
$PNum   = $DB->num_rows($P_Query);
$M_Sql = "select * from `{$INFO[DBPrefix]}bonusalert` where alertdate='" . date("Y-m-d",time()) . "'";	
$M_Query = $DB->query($M_Sql);
$M_Result = $DB->fetch_array($M_Query);
$MNum   = $DB->num_rows($M_Query);
if ($MNum<=0){
	while($Rs = $DB->fetch_array($P_Query)){
		if ($FUNCTIONS->validate_email($Rs['email'])){
			  ...
			  $sumpoint_list[$ui] = $Rs['sumpoint'];
			  $ui++;
		 }	
	}
}else{
	echo "<script>alert('今日已經提醒過了');history.back(-1)</script>";	
	exit;
}


if(is_array($mail_list)){
	$mailstr = implode(",",$mail_list);	
}
...
foreach($mail_list as $k=>$v){
	$Query = $DB->query(" insert into `{$INFO[DBPrefix]}falsemail` (mail,pid,no,sendtime) values ('" . str_replace("\r","",str_replace("\n","",str_replace("\\","",str_replace("\"","",$v)))) . "||" . $user_list[$k] . "||" . $tel_list[$k] . "||"  . $sumpoint_list[$k] . "','0','0','')");
}

$Sql = "insert into `{$INFO[DBPrefix]}bonusalert` (alertdate,alertcount,maillist) values ('" . date("Y-m-d",time()) . "','" . $PNum . "','" . $mailstr . "')";
$Query = $DB->query($Sql);
?>
...
<TITLE>會員管理-->積分管理-->積分過期提醒</TITLE></HEAD>
          ...
          <TD width="50%">
            <TABLE width="90%" border=0 cellPadding=0 cellSpacing=0>
              ...
              <TR>
                ...
                <TD class=p12black noWrap><span class="p9orange">會員管理--&gt;積分管理--&gt;積分過期提醒</span></TD>
              </TR>
			  </TBODY>
			 </TABLE>
		  </TD>
...
function getSendmail() { 
	...
	var url = "admin_pointalert_ajax_sendmail.php";
	var result ;
	var mailno;
	mailno = 1;
