...
if (isset($_GET['kid'])&&intval($_GET['kid'])!=0) {
	...
	$Sql_linshi = " select * from `{$INFO[DBPrefix]}kefu` where kid = $_GET[kid]";
	...
	if ($kefu['iflogin']==1&&empty($_SESSION['user_id'])) {
		...
		$kefu_post_list = <<<EOF
                        <TR>
                          <TD width="25%" height="24" align=left valign="middle">$KeFu_Pack[Report]：</TD>
...
/**
	...
	if ($kefu['postnum']==0) {
		$kefu_post_list = <<<EOF
                        <TR>
                          <TD width="25%" height="24" align=right valign="middle">{$KeFu_Pack[Report]}：</TD>
                          <TD>$KeFu_Pack[No_Report]</TD>
                        </TR>
EOF;
}
/**
	...
	foreach ($kefu_posts as $value) {
		...
		$kefu_post_list .= <<<EOF
                        <TR>
                          <TD width="25%" height="24" align=right valign="middle" bgcolor="#FFFFFF">{$KeFu_Pack[Report]} # {$i}： <br>
                          $value[username]<br>
                          $kefu_post_date
                          </TD>
                          <TD bgcolor="#FFFFFF"><b>$value[k_post_title]</b> <br>
                          $kefu_post_con
                          </TD>
                        </TR>
EOF;
