...
$Sql      = "select u.memberno,u.user_id,u.username,u.true_name,COUNT(t.order_serial) ordercount,SUM(t.totalprice) totalprice from `{$INFO[DBPrefix]}user` u left join `{$INFO[DBPrefix]}order_table` t on u.`user_id`=t.`user_id` WHERE `createtime` BETWEEN UNIX_TIMESTAMP('".$goods_starttime."') AND UNIX_TIMESTAMP('".$goods_endtime." 23:59:59') AND t.order_state!=3 AND t.pay_state!=2 GROUP BY u.user_id ORDER BY t.order_serial desc";
...
<TITLE><?php echo $JsMenu[Functions];//功能?>--&gt;會員統計--&gt;會員消費紀錄列表</TITLE></HEAD>
              ...
              <TABLE width="90%" border=0 cellPadding=0 cellSpacing=0>
                <TBODY>
                  <TR>
                    ...
                    <TD class=p12black noWrap><SPAN class=p9orange><?php echo $JsMenu[Functions];//功能?>--&gt;會員統計--&gt;會員消費紀錄列表</SPAN>
                    </TD>
                </TR></TBODY></TABLE></TD>
          </TR>
        </TBODY>
              ...
              <?php echo $FUNCTIONS->select_Cyc_array("listnum",$limit,"  class=\"trans-input\" onchange=document.form1.submit(); ",$Array=array('2','10','15','20','30','50','100'))?>		  </TD>
					...
					<TR>
						...
						<TD height="30" colspan="2" align=left class=p9black>
							...
							<input type="button" value="匯出" onclick="javascript:location.href='admin_member_consumption_record_list_csv.php?<?php echo "goods_starttime=" . $goods_starttime . "&goods_endtime=" . $goods_endtime; ?>'"/>
						</TD>
						</TR>
        </FORM>
  </TABLE>
                      ...
                      <FORM name=adminForm action="" method=post>
                        ...
                        <TBODY>
                          <TR align=middle>
                            ...
                            <TD width="3%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1>訂單數量</TD>
														<TD width="3%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1>訂單總金額</TD>
                          ...
                          <?php
                            ...
                            <TR class=row0>
                              ...
                              <TD align="left" noWrap><?php echo $Rs['true_name']?></TD>
                              <TD align="center" noWrap><?php echo $Rs['ordercount']?></TD>
															<TD align="center" noWrap><?php echo $Rs['totalprice']?></TD>
															<TD align="center" noWrap><div class="link_box"><a href="admin_member_consumption_record.php?<?php echo "memberno=" . $Rs['memberno'] . "&goods_starttime=" . $goods_starttime . "&goods_endtime=" . $goods_endtime; ?>">明細查詢</a></div></TD>
                            </TR></tbody>
						          <?php
											$i++;
											}
