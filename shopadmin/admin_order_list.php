...
if ($_GET['action']=='search') {
	if (trim(urldecode($_GET['skey']))!=$Basic_Command['InputKeyWord']   && trim($_GET['skey'])!=""){
		...
		}elseif (trim($_GET['type'])=='o.Departure'){
			 $Add_one = " and CONCAT(o.flightid,o.flightno)='" . trim(urldecode($_GET['skey'])) . "' ";
	...
	if ($_GET['flightdate']!=""){
		$Add_three  .=  " and date_format(o.flightdate,'%Y-%m-%d')='" . $_GET['flightdate'] . "' ";
	}else{
		//$_GET['flightdate'] = date("Y-m-d",time());
		//$Add_three  .=  " and date_format(o.flightdate,'%Y-%m-%d')='" . date("Y-m-d",time()) . "' ";
	}
	...
	if (trim($_GET['type'])=='od.goodsname'){
	 ...
	 $Sql = " select  u.true_name,u.en_firstname,u.en_secondname,o.flightid,o.flightno,o.flightdate,o.order_serial,o.order_id,o.createtime,o.totalprice,o.transport_price,
			 ...
			 if(trim($_GET['State'])=="NoOp")
			 	...
			 	$Sql      .= " o.createtime>='$begtimeunix' and o.createtime<='$endtimeunix' ";
...
//echo $Sql;
	...
	if(trim($_GET['State'])=="NoOp"||trim($_GET['State'])=="Cancel"||trim($_GET['State'])=="Back"){
	  ...
	  $sSql      = " o.createtime>='$begtimeunix' and o.createtime<='$endtimeunix' ";
	   $_GET['flightdate'] = date("Y-m-d",time());
   }

	$Add  = $_GET['State']!=""  ? str_replace("and","where",$Add)." and  " . $sSql :  " where  " . $sSql;
	if(trim($_GET['State'])=="today"){
	   $Add      = " where o.createtime>='$begtimeunix' and o.createtime<='$endtimeunix' ";
		$_GET['flightdate'] = "";
   }
	if(trim($_GET['State'])=="todayTrans"){
	   $Add      = " where date_format(o.flightdate,'%Y-%m-%d')='" . date("Y-m-d",time()) . "' ";
		$_GET['flightdate'] = date("Y-m-d");
   }

	$Add  = $_GET[Order_Tracks]=="Show" ? $Add." and o.order007_status=1 and  o.order007_begtime <= '".date("Y-m-d",time())."' " : $Add ;

	if(!(trim($_GET['State'])=="NoOp"||trim($_GET['State'])=="Cancel"||trim($_GET['State'])=="Back"||trim($_GET['State'])=="today"))
		$Add  .=  " and date_format(o.flightdate,'%Y-%m-%d')='" . date("Y-m-d",time()) . "' ";
	 $Sql  = " select u.true_name,u.en_firstname,u.en_secondname,o.flightid,o.flightno,o.flightdate,o.order_serial,o.order_id,o.createtime,o.totalprice,o.transport_price,o.invoice_code,o.MallgicOrderId,
...
function toEdit(id){
	...
	if (checkvalue!=false){
		...
		document.adminForm.action = "admin_order_detail.php?Action=Modi&order_id="+id;
...
<TABLE class=allborder cellSpacing=0 cellPadding=0 width="100%"  align=center border=0 style="margin-top:10px;margin-bottom:10px">
                  ...
                  <TR>
                    <TD height=31 colspan="3" align=left nowrap>
                      <SELECT name='type' class="trans-input" >
                        ...
                        <OPTION    value="o.Departure" <?php if ($_GET['type']=='o.Departure') echo " selected ";?>>班機</OPTION>
		  ...
		  ?>
                      ...
                      <!--<span class="p9black">&nbsp;<?php echo $Good[ProviderNameText] ;?><!--供貨商--><!--</span>：<?php echo $FUNCTIONS->select_type("select provider_name,provider_id from `{$INFO[DBPrefix]}provider` order by provider_id asc  ","provider_id","provider_id","provider_name",intval($_GET['provider_id']));  ?>-->&nbsp;&nbsp; 下單日期：From
                      ...
                      &nbsp;&nbsp; 提貨日期：
                      <INPUT    id=flightdate size=10 value="<?php echo $_GET['flightdate']?>"      onclick="showcalendar(event, this)" onFocus="showcalendar(event,this);if(this.value=='0000-00-00')this.value=''"  name=flightdate />
                ...
                <TR>
                  <TD bgColor=#ffffff>
                      ...
                      <FORM name=adminForm id=adminForm action="" method=post>
                        ...
                        <INPUT type=hidden name=sendname id=sendname>
                          ...
                          <TR align=middle>
                            ...
                            <TD width="7%"  height=26 align="left" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1> <?php echo $Order_Pack[OrderSerial_say];//订单号?></TD>
                            ...
                            <TD width="7%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1>班機時間</TD>
                            <TD width="6%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1>航班編號</TD>
                            <TD width="7%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1>消費總金額</TD>
                            ...
                            <TD width="6%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1> 取貨人中文姓名</TD>
                            <TD width="6%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black style1> 取貨人英文姓名</TD>
                            <!--TD width="5%" align="center" noWrap background=images/<?php echo $INFO[IS]?>/bartop.gif class=p9black>發票號碼</TD-->
					...
					?><tbody>
                      ...
                      <!--?php if ($Rs['order_serial_together']!="") echo"<a href=\"javascript:showWin('url','admin_order_together.php?order_id=" . $Rs['order_id'] . "','',300,250);\">" . $Rs['order_serial_together'] . "</a>"; else echo "<div class=link_box><a href=\"javascript:showWin('url','admin_order_together.php?order_id=" . $Rs['order_id'] . "','',300,250);\">填寫</a></div>";?> a</TD-->
                              ...
                              <TD height=26 align="center" noWrap><?php echo $Rs['flightdate']?></TD>
                              <TD height=26 align="center" noWrap><?php echo $Rs['flightid'].$Rs['flightno']?></TD>
                              ...
                              <TD height=26 align="center" noWrap><?php echo $Rs['en_firstname'] . " " . $Rs['en_secondname']?></TD>
