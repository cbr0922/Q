...
$Query = $DB->query(" delete from `{$INFO[DBPrefix]}falsesmg` where no='0' and sendtime=''");
...
$Query_time = $DB->query(" update `{$INFO[DBPrefix]}edmsmg` set publication_end_time='".time()."' where publication_id=".intval($Pub_id));
$mi = 0;
for ($i=0;$i<$Num_Mgroup;$i++){
	/*
	$Query = $DB->query(" select other_tel,user_level from `{$INFO[DBPrefix]}user` where user_level=".intval($Mgroup[$i])." limit 0,1" );
	while ($Rs = $DB->fetch_array($Query)){
		$mail_list = array_merge($mail_list,explode(",",$Rs[other_tel]));
		$mi++;
	}
	*/
	//郵件組中的郵件列表
	...
	if($Rs['searchlist']=="All"){
		...
		$Sql      = "select m.*,u.other_tel from `{$INFO[DBPrefix]}mail_group_list` as m inner join `{$INFO[DBPrefix]}user` as u on m.user_id=u.user_id where group_id='" . intval($Mgroup[$i]) . "' and u.other_tel<>'' order by m.email asc ";
	}
	$Query = $DB->query($Sql);
	$Num   = $DB->num_rows($Query);
	if ($Num>0){
		while ($Rs_sub = $DB->fetch_array($Query)){
			$mail_list[$i] = MD5Crypt::Decrypt ($Rs_sub['other_tel'], $INFO['mcrypt']);
			$i++;
		}
	}
}
...
$sql = "select max(no) as maxno from `{$INFO[DBPrefix]}falsesmg`";
$Query = $DB->query($sql);
$Result= $DB->fetch_array($Query);
$maxno = intval($Result[maxno])+1;

foreach($mail_list as $v){
	$Query = $DB->query(" insert into `{$INFO[DBPrefix]}falsesmg` (tel,pid,no,sendtime) values ('" . str_replace("\r","",str_replace("\n","",str_replace("\\","",str_replace("\"","",$v)))) . "','0','0','')");
}
?>
          ...
          <TD width="50%">
            <TABLE width="90%" border=0 cellPadding=0 cellSpacing=0>
              ...
              <TR>
                ...
                <TD class=p12black noWrap><SPAN  class=p9orange><?php echo $JsMenu[Functions];//功能?>--&gt;<?php echo $JsMenu[Shop_Pager];//网店会刊?>--&gt;發送簡訊
                </SPAN></TD>
              </TR>
			  </TBODY>
			 </TABLE>
                      ...
                      <TD align=middle width=79><!--BUTTON_BEGIN-->
                        <TABLE>
                          ...
                          <TR>
							...
							<a href="admin_edmsmg_list.php"><IMG  src="images/<?php echo $INFO[IS]?>/fb-cancel.gif" border=0>&nbsp;<?php echo $Basic_Command['Return']; ?></a>&nbsp;
							</TD>
						  </TR>
						  </TBODY>
						</TABLE><!--BUTTON_END-->
      ...
      <TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
        ...
        <form name="form1" method="post" action="admin_pubsend_act.php">
          ...
          <tr>
            ...
            <td height="40">總共需要發送<?=$totalnum?>條簡訊</td>
          </tr>
          <tr>
          ...
          <tr id="tr1">
            <td height="50" valign="top" width="20%" align="right">已發送的簡訊</td>
...
function getSendmail() {
	...
	var url = "admin_ajax_sendsmg.php";
	var result ;
	var mailno;
	mailno = 1;
