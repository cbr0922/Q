			...
			//if ($TrueHave==0){
				...
				if (time($data[7])!=""){
					...
					if ($Num<=0){
						...
						$Query_b = $DB->query("select bn from `{$INFO[DBPrefix]}goods` where provider_id=".intval($data[2])." order by gid desc limit 0,1");
						$Result_b= $DB->fetch_array($Query_b);
					 ...
					 'bn'                 => trim($data[7]),
					  ...
					  'weight'              => ($data[77]),
						...
						$Sql = "UPDATE `{$INFO[DBPrefix]}goods` SET $db_string  WHERE bn='".trim($data[7]) . "'";
