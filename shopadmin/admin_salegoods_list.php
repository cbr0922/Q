...
$Sql      = "select  g.*,bc.catname from `{$INFO[DBPrefix]}goods` g  left join `{$INFO[DBPrefix]}bclass`  bc  on (g.bid=bc.bid)    where g.ifsales=1  ".$Where." ";
$Add      = !empty($_GET['subject_id']) && intval($_GET['subject_id'])!=0 ? " and g.sale_subject=".intval($_GET['subject_id']) : "" ;
...
$Table_sub = "
<select name='subject_id'  onchange=\"window.location=('admin_salegoods_list.php?subject_id='+this.value);\"   class=\"trans-input\" >
...
<title><?php echo $JsMenu[Product_Man];//商品管理?>--&gt;<?php echo $JsMenu[Subject];?>--&gt;<?php echo $Table_sub?></title>
</HEAD>
        ...
        <FORM name=optForm method=get action="">        
          ...
          <TR>
            <TD align=left colSpan=2 height=31>
              <TABLE class=p12black cellSpacing=0 cellPadding=0 width=344 border=0>
                <TBODY>
                  <TR>
                    <TD align=right width=280 height=31>
                      <INPUT  name='skey'   onfocus=this.select()  onclick="if(this.value=='<?php echo $Admin_Product[PleaseInputPrductName]?>')this.value=''"  onmouseover=this.focus() value='<?php echo $Admin_Product[PleaseInputPrductName]?>' size="40"> </TD>
                    <TD align=middle width=6 height=31>
                      
                    </TD>
                    <TD class=p9black vAlign=center width=58 height=31> <INPUT type=image src="images/<?php echo $INFO[IS]?>/t_go.gif" border=0 name=imageField align="absmiddle"> 
                    </TD>
                  </TR>
                </TBODY>
              </TABLE>
