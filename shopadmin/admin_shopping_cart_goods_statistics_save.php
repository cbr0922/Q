...
$filename="購物車商品(".$_GET['gid'].")統計_".date("Y-m-d",time());
...
$Sql = "select u.user_id, u.email from `{$INFO[DBPrefix]}user` u left join `{$INFO[DBPrefix]}shopping` as s on u.user_id=s.user_id where u.user_id!=0 and s.gid='".$_GET['gid']."' group by s.user_id";
$Query    = $DB->query($Sql);
