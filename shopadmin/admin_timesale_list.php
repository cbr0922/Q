...
$Sql      = "select  g.*,bc.catname from `{$INFO[DBPrefix]}goods` g  left join `{$INFO[DBPrefix]}bclass`  bc  on (g.bid=bc.bid)  where (g.iftimesale=1 or g.ifsaleoff=1)  ".$Where." ";
...
function toEdit(id,catid){
	...
	if (checkvalue!=false){		
    	...
    	document.adminForm.act.value="";
   	    document.adminForm.Where.value="Timesale";		
		document.adminForm.submit();
	}
}
              ...
              <TABLE cellSpacing=0 cellPadding=0 border=0>
                <TBODY>
                  <TR>
                    ...
                    <TD class=p12black noWrap><SPAN class=p9orange><?php echo $JsMenu[Product_Man];//商品管理?>--&gt;整點促銷商品列表</SPAN></TD>
              </TR></TBODY></TABLE></TD>
