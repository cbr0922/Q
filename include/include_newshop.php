...
$newshop_array = array();
$i = 0;
$Sql      = "select s.*,u.username,u.true_name from `{$INFO[DBPrefix]}shopinfo` s  left join `{$INFO[DBPrefix]}user` u on (s.uid = u.user_id) where s.state=1 order by rand(),s.sid desc limit 0,1";
$Query    = $DB->query($Sql);
...
$tpl->display("include_newshop.html");
?>
