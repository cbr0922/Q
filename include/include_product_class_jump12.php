...
$Sql_bclass    = "select bid,catname,pic1,pic2,top_id from `{$INFO[DBPrefix]}bclass` where catiffb=1 and bid='" . intval($_GET['bid']) . "'  order by catord,bid  asc limit 0,1 ";
...
//if (intval($_GET['bid'])==0){
	...
	$Sql_bclass    = "select bid,catname,pic1,pic2,subject_id,subject_id2 from `{$INFO[DBPrefix]}bclass` where catiffb=1 and top_id=0 order by catord,bid  asc";

//}else{

//	$Sql_bclass    = "select bid,catname,pic1,pic2,subject_id,subject_id2 from `{$INFO[DBPrefix]}bclass` where catiffb=1  and bid='" . intval($show_top_id) . "' order by catord  asc  ";
...
while ($Rs_bclass =  $DB->fetch_array($query_bclass)){
	...
	$Sql_bclass_2    = "select bid,catname,subject_id,subject_id2 from `{$INFO[DBPrefix]}bclass` where catiffb=1 and top_id='" . $Rs_bclass['bid'] . "' order by catord  asc  ";
...
$tpl->display("include_product_class_jump12.html");
