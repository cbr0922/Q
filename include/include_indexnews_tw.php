...
$Pic_News     = "select ns.news_id,ns.ntitle,ns.nimg,ns.brief,ns.news_id,ns.nltitle,ns.nltitle_color,ns.url_on,ns.url,ns.nidate,ns.pubdate from `{$INFO[DBPrefix]}nclass` nc inner join  `{$INFO[DBPrefix]}news` ns on ( nc.ncid = ns.top_id ) where nc.ncid='".intval($INFO[IndexNewClassId])."' and  nc.ncatiffb='1' and  ns.niffb='1' order by ns.pubdate desc,ns.nord asc, ns.nidate desc  limit 0,5";
...
while( $Result_Pic_news = $DB->fetch_array($Query_Pic_News) )
{
	 $Sql = "select * from `{$INFO[DBPrefix]}news` n inner join `{$INFO[DBPrefix]}nclass` nc on ( n.top_id=nc.ncid ) where nc.ncid='".intval($INFO[IndexNewClassId])."' order by pubdate desc limit 0,3";
	$Query   = $DB->query($Sql);
	$i = 0;
	$Pic_News = array();
...
$tpl->display("include_indexnews_tw.html");
?>