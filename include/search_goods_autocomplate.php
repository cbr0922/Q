...
//~ }
//~ 
$sqlcmd = "select goodsname, checkstate, view_num from  `{$INFO[DBPrefix]}goods` g";
...
if ($Num>0){
		while($AutoCompleteRow = $DB->fetch_array($Query)){
				...
				$result = $result.",{\"value\":\"".trim($AutoCompleteRow['goodsname'])."\",\"label\":\"".trim($AutoCompleteRow['goodsname'])."\"}";
		}
}
$result = $result."]";
