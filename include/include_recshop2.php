...
$newshop_array = array();
$i = 0;
$Sql      = "select s.*,u.username,u.true_name from `{$INFO[DBPrefix]}shopinfo` s  left join `{$INFO[DBPrefix]}user` u on (s.uid = u.user_id) where s.ifrecommend=1 and s.state=1 order by s.sid desc limit 0,10";
$Query    = $DB->query($Sql);
...
$tpl->display("include_recshop2.html");
?>
