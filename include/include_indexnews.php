 ...
 *  四組
 */
$Pic_News     = "select ns.news_id,ns.ntitle,ns.nimg,ns.brief,ns.news_id,ns.nltitle,ns.nltitle_color,ns.url_on,ns.url from `{$INFO[DBPrefix]}nclass` nc inner join  `{$INFO[DBPrefix]}news` ns on ( nc.ncid = ns.top_id ) where  nc.ncid='1' and  nc.ncatiffb='1' and  ns.niffb='1' order by ns.nord asc, ns.nidate desc  limit 0,5";
	...
	while( $Result_Pic_news = $DB->fetch_array($Query_Pic_News) )
	{
	   ...
	   $Nltitle_s =  $Result_Pic_news['url_on']==0 ? "<a href='".$INFO['site_url']."/article/article.php?articleid=".intval($Result_Pic_news['news_id'])."' class=job2>".$Nltitle_first."</a>" :  "<a href='".$Result_Pic_news['url']."'>".$Nltitle_first."</a>";
...
//網站焦點結束
$tpl->display("include_indexnews.html");
?>
